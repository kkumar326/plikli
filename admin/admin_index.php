...
/* Redwine: changed the query to mysqli to be compliant with php 5.5+ and added a filter to just get the size of tables belonging to the current kliqqi site */
...
/*$sql = mysql_query("SELECT link_id,link_date FROM " . table_links . " ORDER BY link_date DESC LIMIT 1");
    while ($rows = mysql_fetch_array($sql)) {
		...
		$main_smarty->assign('link_date', $link_date . ' ' . $main_smarty->get_config_vars('KLIQQI_Visual_Comment_Ago'));
...
$sql = mysql_query("SELECT link_id,comment_id,comment_link_id,comment_date FROM " . table_comments . "," . table_links . " WHERE comment_link_id = link_id ORDER BY comment_date DESC LIMIT 1");
	while ($rows = mysql_fetch_array($sql)) {
		...
		$main_smarty->assign('comment_date', $comment_date . ' ' . $main_smarty->get_config_vars('KLIQQI_Visual_Comment_Ago'));
		$main_smarty->assign('link_id', $rows['link_id']);
...
$main_smarty->assign('kliqqi_lang_conf',lang_loc . "/languages/lang_" . kliqqi_language . ".conf");
...
if($widgets){
	...
	for($i=0; $i<sizeof($widgets); $i++) 
	{
		...
		if (file_exists($file)) 
		{
			...
			if (file_exists('../widgets/'.$widgets[$i]['folder'].'/lang_' . kliqqi_language . '.conf'))
			    $widgets[$i]['lang_conf'] = '../widgets/'.$widgets[$i]['folder'].'/lang_' . kliqqi_language . '.conf';
