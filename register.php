...
$navwhere['text1'] = $main_smarty->get_config_vars('KLIQQI_Visual_Breadcrumb_Register');
$navwhere['link1'] = getmyurl('register', '');
$main_smarty->assign('navbar_where', $navwhere);
$main_smarty->assign('posttitle', $main_smarty->get_config_vars('KLIQQI_Visual_Breadcrumb_Register'));
...
$kliqqi_regfrom = isset($_POST["regfrom"]) && sanitize($_POST['regfrom'], 3) != '' ? sanitize($_POST['regfrom'], 3) : '';
if($kliqqi_regfrom != ''){

	$error = false;
	switch($kliqqi_regfrom){
		case 'full':
...
function register_check_errors($username, $email, $password, $password2){
	...
	if(is_ip_banned($userip)) { 
		$form_username_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_YourIpIsBanned');
		$error = true;
	}	

	if(!isset($username) || strlen($username) < 3) { // if no username was given or username is less than 3 characters
		$form_username_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_UserTooShort');
		$error = true;
	}	
	if (preg_match('/\pL/u', 'a')) {	// Check if PCRE was compiled with UTF-8 support
		...
		$form_username_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_UserInvalid');
		$error = true;
	    }
	} else {
	...
	if(user_exists(trim($username)) ) { // if username already exists
		$form_username_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_UserExists');
		$error = true;
	}
	if(!check_email(trim($email))) { // if email is not valid
		$form_email_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_BadEmail');
		$error = true;
	}
	if(email_exists(trim($email)) ) { // if email already exists
		$form_email_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_EmailExists');
		$error = true;
	}
	if(strlen($password) < 5 ) { // if password is less than 5 characters
		$form_password_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_FiveCharPass');
		$error = true;
	}
	if($password !== $password2) { // if both passwords do not match
		$form_password_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_NoPassMatch');
		$error = true;
	}	
...
function register_add_user($username, $email, $password, $password2, $user_language){
	...
	if($user->Create()){
		...
		elseif(kliqqi_validate()){
		    header('Location: '.my_base_url.my_kliqqi_base.'/register_complete.php?user='.$username);
		} else {
