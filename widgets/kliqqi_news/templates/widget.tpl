...
.kliqqi_news_paragraph {
	line-height:1.4em;
}
.kliqqi_news_paragraph strong {
	font-weight:normal;
}
</style>
{/literal}
...
function feedMe($feed,$items) {
	...
	if ($blog_curl_errno > 0) {		
		if ($blog_curl_errno = 28){
			echo 'Unable to connect to the Kliqqi Blog feed. Please try again later.';
		}else{
		...
		for($i=0; $i<$items; $i++)
		{
			...
			if ($title != "" && $desc != "") {
				echo '<h4 class="kliqqi_news_title"><a href="'.$url.'" target="_blank">'.$title.'</a></h4><p class="kliqqi_news_paragraph">'.$desc.'</p>';
			}
		}
	}
}
...
if (strstr(my_base_url,"localhost")) {
	feedMe("http://feeds.feedburner.com/Kliqqi",$items);
}else{
feedMe("https://www.kliqqi.com/feed/",$items);
}
