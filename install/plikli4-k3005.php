...
printf("Affected rows (DELETE): %d\n", $handle->affected_rows);
	...
	printf("Affected rows (UPDATE): %d\n", $handle->affected_rows);
	echo '<li>Updated the description of user language, replacing kliqqi instances with plikli. <img src="'.$marks.'" class="iconalign" /></li>';
...
//start inserting new entries in the misc_data table
echo '<fieldset><legend>Inserting new entries in the misc_data table</legend><ul>';	
	...
	if (!$sql_karma_story_unvote) {
		...
		$marks = $ok;
			...
			printf("Affected rows (INSERT): %d\n", $handle->affected_rows);
	echo '<li>Inserted karma_story_unvote value and relevant entries to modules and plikli version update in "Misc_data" table <img src="'.$marks.'" class="iconalign" /></li>';
echo '</ul></fieldset><br />';
//end inserting new entries in the misc_data table
...
$folder_path = $fetched['var_value'];
	...
	while ($module = $sql_modules->fetch_assoc()) {
		...
		if ($module['folder'] == 'xml_sitemaps') {
			...
			//Update the description of XmlSitemaps
			$sql = "update `" . table_prefix . "config` set `var_desc` = replace(`var_desc`, '../sitemapindex.xml', '../module.php?module=xml_sitemaps_show_sitemap&i=main') WHERE `var_page` = 'XmlSitemaps' AND `var_optiontext` = 'link'";
			$sql_XmlSitemaps = $handle->query($sql);
			if (!sql_XmlSitemaps) {
				$marks = $notok;
			}else{
				$marks = $ok;
			...
			printf("Affected rows (UPDATE): %d\n", $handle->affected_rows);
			echo '<li>Updated description of XmlSitemaps <img src="'.$marks.'" class="iconalign" /></li>';
		}
