...
if ($old_version < $new_version) {
	...
	if (count($result) == 0) {
		$db->query("INSERT INTO `" . table_config . "` ( `var_id` , `var_page` , `var_name` , `var_value` , `var_defaultvalue` , `var_optiontext` , `var_title` , `var_desc` , `var_method` , `var_enclosein` )VALUES (NULL, 'Misc', 'misc_validate', 'false', 'false', 'true / false', 'Validate user email', 'Require users to validate their email address?<br />If you set to true, then click on the link below to also set the email to be used for sending the message.<br /><a href=\"../module.php?module=admin_language\">Set the email</a>. Type @ in the filter box and click Filter to get the value to modify. Do not forget to click save.', 'define', '');");
	}
	...
	if (!$fieldexists) {
		$sql = "ALTER TABLE  `".table_links."` ADD  `link_group_status` SET(  'new',  'published',  'discard' ) DEFAULT 'new' NOT NULL ;";
		$db->query($sql);
	}
	...
	if (!$fieldexists) {
		...
		$sql = "ALTER TABLE `".table_users."` CHANGE  `user_categories`  `user_categories` VARCHAR( 255 ) DEFAULT  ''";
		$db->query($sql);
...
/* Redwine: using php explode() instead of the deprecated split() which generates warnings. */
			    $cats = explode(',',$row['user_categories']);
