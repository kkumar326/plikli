...
<legend>
	{if $templatelite.get.user}
		{$templatelite.get.user|sanitize:2}'s {#KLIQQI_Visual_TopUsers_TH_News#}
	{else}
		{#KLIQQI_Visual_AdminPanel_Links#}
	{/if}
</legend>
	...
	<div class="col-md-3">
		<div class="btn-group pull">
			...
			<ul class="dropdown-menu">
				<li><a onclick="set_admin_action('published')" href="#"><i class="fa fa-level-up"></i>{#KLIQQI_Visual_AdminPanel_Publish#}</a></li>
				<li><a onclick="set_admin_action('new')" href="#"><i class="fa fa-level-down"></i>{#KLIQQI_Visual_AdminPanel_New#}</a></li>
				<li><a onclick="set_admin_action('discard')" href="#"><i class="fa fa-trash-o"></i>{#KLIQQI_Visual_AdminPanel_Discard#}</a></li>
				<li><a onclick="set_admin_action('moderated')" href="#"><i class="fa fa-ban"></i>{#KLIQQI_Visual_AdminPanel_Moderated#}</a></li>
				<li><a onclick="set_admin_action('spam')" href="#"><i class="fa fa-ban"></i>{#KLIQQI_Visual_AdminPanel_Spam#}</a></li>
			</ul>
		</div>
	</div>
	<form action="{$my_base_url}{$my_kliqqi_base}/admin/admin_links.php" method="get" class="form-search search-bar">
		<div class="col-md-3">
			<div class="input-append">
				...
				<button type="submit" class="btn btn-primary search-button">{#KLIQQI_Visual_Search_Go#}</button>
			</div>
		</div>
		<div class="col-md-3">
...
<form name="bulk_moderate" id="user_list_form" action="{$my_base_url}{$my_kliqqi_base}/admin/admin_links.php?action=bulkmod&page={$templatelite.get.page|sanitize:2}" method="post" onsubmit="return validate_all_user_action()">
	...
	<table class="table table-bordered table-condensed">
		<tr>
			...
			{checkActionsTpl location="tpl_kliqqi_admin_submissions_th_start"}
			<th>{#KLIQQI_Visual_View_Links_Author#}</th>
			<th>{#KLIQQI_Visual_View_Links_New_Window#}</th>
			<th nowrap style="text-align:center;">{#KLIQQI_Visual_AdminPanel_Group_Date#}</th>
			<th nowrap style="text-align:center;">{#KLIQQI_Visual_View_Links_Status#}</th>
			{checkActionsTpl location="tpl_kliqqi_admin_submissions_th_end"}
		</tr>
		...
		<tr {if $template_stories[id].link_status=='moderated'}class="tr_moderated"{/if}>
			...
			{checkActionsTpl location="tpl_kliqqi_admin_submissions_td_start"}
			<td><a href="{$my_base_url}{$my_kliqqi_base}/admin/admin_users.php?mode=view&user={$template_stories[id].link_author}" title="{$template_stories[id].link_author}'s Articles" id="link-{$template_stories[id].link_id}-author">{$template_stories[id].link_author}</a></td>
			<td>
				<a href='{$my_kliqqi_base}/editlink.php?id={$template_stories[id].link_id}'><i class="fa fa-edit" title="{#KLIQQI_Visual_AdminPanel_Page_Edit#}" alt="{#KLIQQI_Visual_AdminPanel_Page_Edit#}"></i></a>
				<a href="{$my_base_url}{$my_kliqqi_base}/story.php?title={$template_stories[id].link_title_url}" title="{$template_stories[id].link_title|truncate:50:"...":true}" >{$template_stories[id].link_title}</a>
			</td>
			<td style="text-align:center;vertical-align:middle;">{$template_stories[id].link_date}</td>
			<td style="text-align:center;vertical-align:middle;">
				{if $template_stories[id].link_status=='new'} 
					{#KLIQQI_Visual_AdminPanel_New#}
				{elseif $template_stories[id].link_status=='published'}
					{#KLIQQI_Visual_AdminPanel_Published#}
...
<div style="float:right;margin-top:6px;">
	<a data-toggle="modal" href="{$my_base_url}{$my_kliqqi_base}/admin/admin_delete_stories.php" class="btn btn-danger"><i class="fa fa-trash-o fa-white"></i> {#KLIQQI_Visual_AdminPanel_Delete_Stories#}</a>
</div>
<div style="clear:both;"> </div>
<SCRIPT>
