					...
					<table class="table table-bordered table-striped vertical-align">
						<thead class="table_title">
							<tr>
								...
								{if !empty($group_display)}<th style="width:60px;text-align:center;">{#KLIQQI_Visual_Group_Member#}</th>{/if}
							</tr>
						<tbody>
			...
			{if $Allow_Friends neq "0"}
				<div id="following" class="col-md-6">
					<table class="table table-bordered table-striped vertical-align">
						<thead class="table_title">
							<tr>
								<th>{#KLIQQI_Visual_User_Profile_Friends#}</th>
									...
									<tr>
										...
										{if $user_authenticated eq true && $myfriend.is_mutual eq 'mutual'}
											<td style="text-align:center">
												<a href="{$my_kliqqi_base}/module.php?module=simple_messaging&view=compose&return={$templatelite.server.REQUEST_URI|urlencode}&to={$myfriend.user_login}" class="btn btn-default"><i class="fa fa-envelope"></i></a>
											</td>
										...
										{if $user_authenticated eq true && $myfriend.is_mutual eq 'mutual' || $myfriend.is_mutual eq 'following'}
											<td>
												<a class="btn btn-danger" href="{$removeURL}">{#KLIQQI_Visual_User_Profile_Remove_Friend#}</a>
											</td>
										{elseif $user_authenticated neq true}
										
										{else}
											<td>
												<a class="btn btn-success" href="{$addURL}">{#KLIQQI_Visual_User_Profile_Add_Friend#}</a>
											</td>											
										{/if}
									</tr>
								{/foreach}
