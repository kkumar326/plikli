...
<form action="" method="post" id="thisform" enctype="multipart/form-data" >
	...
	<legend>{#KLIQQI_Visual_EditStory_Header#}: {$submit_title}</legend>
	<strong>{#KLIQQI_Visual_Submit2_NewsURL#}: </strong>
	...
	<br />
	{* 
		<strong for="url_title" accesskey="2">{#KLIQQI_Visual_Submit2_URLTitle#}: </strong>{$submit_url_title} 
		<hr />
	*}
	<strong>{#KLIQQI_Visual_Submit2_Title#}: </strong> 
	<span class="field-description">{#KLIQQI_Visual_Submit2_TitleInstruct#}</span>
	<br />
	...
	{if $isAdmin eq 1 or $isModerator eq 1}
		<strong>{#KLIQQI_Visual_Change_Link_Submitted_By#}: </strong>
		<br />
...
{* Redwine: Roles and permissions and Groups fixes. To add the ability to submit to a group or to change from one group to another *}
	...
	{/if}
	{if $enable_tags}
		<strong>{#KLIQQI_Visual_Submit2_Tags#}: </strong> 
		<span class="field-description">{#KLIQQI_Visual_Submit2_Tags_Inst1#} {#KLIQQI_Visual_Submit2_Tags_Example#} {#KLIQQI_Visual_Submit2_Tags_Inst2#}</span>
		...
		<br />
	{/if}
	<strong>{#KLIQQI_Visual_Submit2_Description#}: </strong>
	<span class="field-description">{#KLIQQI_Visual_Submit2_DescInstruct#}</span>
	<br />
	...
	{if $Story_Content_Tags_To_Allow neq "" && $enable_tags}
		<span class="help-inline">
			{#KLIQQI_Visual_Submit2_HTMLTagsAllowed#}: {$Story_Content_Tags_To_Allow}
		</span>
	{/if}
	<br />
	{if $SubmitSummary_Allow_Edit eq 1}
		<br />
		<strong>{#KLIQQI_Visual_Submit2_Summary#}: </strong>
	...
	<span class="field-description">{#KLIQQI_Visual_Submit2_CatInstruct#}</span>
	<br />
...
{* Redwine: to apply the proper CSS Class depending on whether the multi categories is set or not *}
	...
	{if $canIhaveAccess eq 1}
		<strong>{#KLIQQI_Visual_EditStory_Notify#}: </strong>
		<br />
		&nbsp; <input type="checkbox" name="notify" value="yes"> {#KLIQQI_Visual_EditStory_NotifyText#}
		<ul class="notify_option_list">
			<li><input type="radio" name="reason" value="typo" onclick="notify.checked = 'true';">{#KLIQQI_Visual_EditStory_Reason_typo#}</li>
			<li><input type="radio" name="reason" value="category" onclick="notify.checked = 'true';">{#KLIQQI_Visual_EditStory_Reason_category#}</li>
			<li><input type="radio" name="reason" value="tags" onclick="notify.checked = 'true';">{#KLIQQI_Visual_EditStory_Reason_tags#}</li>
			<li><input type="radio" name="reason" value="foul" onclick="notify.checked = 'true';">{#KLIQQI_Visual_EditStory_Reason_foul#}</li>
			<li>
				<input type="radio" name="reason" value="other" onclick="notify.checked = 'true';">{#KLIQQI_Visual_EditStory_Reason_other#} 
