...
$navwhere['text1'] = $main_smarty->get_config_vars('KLIQQI_Visual_Breadcrumb_Recover_Password');
$navwhere['link1'] = getmyurl('loginNoVar', '');
$main_smarty->assign('navbar_where', $navwhere);
$main_smarty->assign('posttitle', $main_smarty->get_config_vars('KLIQQI_Visual_Breadcrumb_Recover_Password'));
...
if($user){
	if( (isset($_POST["processrecover"]) && is_numeric($_POST["processrecover"]))){
		...
		if($_POST["processrecover"]==1){
			...
			if ($hashed_fields != $n || $secret != $secret_code) {
				$form_password_error[] = $main_smarty->get_config_vars('KLIQQI_Visual_Register_Error_Secret_Code');
				$error = true;
			...
			if($error==false){
				...
				$body = sprintf(
					$main_smarty->get_config_vars("KLIQQI_PassEmail_Reset_Success"),
	...
	$errorMsg = $main_smarty->get_config_vars('KLIQQI_Password_Invalid_URL');
