...
<table class="upload_ajax_table">
		...
		{/php}
			...
			<tr class="upload_ajax_image">
				<td>
					{php}
						...
						$image =  $this->_vars['my_base_url'].$this->_vars['my_kliqqi_base'].'/'.$this->_vars['upload_thdirectory'].'/'.$this->_vars['image']['file_name'];
						// echo $image;
					...
					{elseif $image.file_size=='orig'}
						<a href="{$my_kliqqi_base}{$upload_directory}/{$image.file_name}" target="_blank"><img  src='{$my_kliqqi_base}{$upload_directory}/{$image.file_name}' height="{$image_height}" width="{$image_width}" class="upload_ajax_image_preview"  style='max-width:520px;margin-top:5px'/></a>
					{else}
						<img src='{$my_kliqqi_base}{$upload_thdirectory}/{$image.file_name}' height="{$image_height}" width="{$image_width}" class="upload_ajax_image_preview"  style='max-width:520px;margin-top:5px'/>
					{/if}
				</td>
			</tr>
			{*
			<tr class="upload_ajax_details">
				...
				<td class="upload_ajax_instructions">
					<span style="font-weight:normal;">{#KLIQQI_Upload_Code_Instructions#}</span><br />
	...
	{if $upload_allow_hide}
		<tr class="upload_ajax_hide">
			<td>
				{if $ispicture}
					<input type='checkbox' onclick='switchImage({$submit_id},{$number},"thumb");' {if $hide_thumb}checked{/if}> {#KLIQQI_Upload_Off_Thumb#}<br>
				{/if}
				<input type='checkbox' onclick='switchImage({$submit_id},{$number},"file");'  {if $hide_file}checked{/if}> {#KLIQQI_Upload_Off_File#}
			</td>
		</tr>
	{/if}
