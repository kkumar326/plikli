...
<legend>{#plikli_RSS_Import#}</legend>
...
function do_the_import_stuff($feed) { 
	...
	if(count($Feed_Links) > 0){
		foreach ($Feed_Links as $link ) {
			if($link->plikli_field == 'link_title'){$MyArray['title'] = $link->feed_field;}
			if($link->plikli_field == 'link_content'){$MyArray['content'] = $link->feed_field;}
			if($link->plikli_field == 'link_url'){$MyArray['link_url'] = $link->feed_field;}
			if($link->plikli_field == 'link_tags'){$MyArray['link_tags'] = $link->feed_field;}
			if($link->plikli_field == 'link_field1'){$MyArray['link_field1'] = $link->feed_field;}
			if($link->plikli_field == 'link_field2'){$MyArray['link_field2'] = $link->feed_field;}
			if($link->plikli_field == 'link_field3'){$MyArray['link_field3'] = $link->feed_field;}
			if($link->plikli_field == 'link_field4'){$MyArray['link_field4'] = $link->feed_field;}
			if($link->plikli_field == 'link_field5'){$MyArray['link_field5'] = $link->feed_field;}
			if($link->plikli_field == 'link_field6'){$MyArray['link_field6'] = $link->feed_field;}
			if($link->plikli_field == 'link_field7'){$MyArray['link_field7'] = $link->feed_field;}
			if($link->plikli_field == 'link_field8'){$MyArray['link_field8'] = $link->feed_field;}
			if($link->plikli_field == 'link_field9'){$MyArray['link_field9'] = $link->feed_field;}
			if($link->plikli_field == 'link_field10'){$MyArray['link_field10'] = $link->feed_field;}
			if($link->plikli_field == 'link_field11'){$MyArray['link_field11'] = $link->feed_field;}
			if($link->plikli_field == 'link_field12'){$MyArray['link_field12'] = $link->feed_field;}
			if($link->plikli_field == 'link_field13'){$MyArray['link_field13'] = $link->feed_field;}
			if($link->plikli_field == 'link_field14'){$MyArray['link_field14'] = $link->feed_field;}
			if($link->plikli_field == 'link_field15'){$MyArray['link_field15'] = $link->feed_field;}
		}
...
// MJE: MOD to include title words as tags ----------------------------
			if (trim($linkres->tags) == '') { 
				$temp1 = mb_strtolower($linkres->title, 'UTF-8');
				...
				foreach ($temp2 as $key => $str)
				{
					...
					unset($temp2[$key]);
					{* Redwine: added the below code to prevent inserting tags less than 3 characters and to make sure the tags don't exceed the maxTagsLength set in the Dashboard -> Settings -> Submit -> Maximum Tag Line Length. So I truncate the tags to the last comma before the Maximum Tag Line Length. *}
					if (mb_strlen($str, 'utf8') > 3) {
						if(!in_array($str,$temp3)){
						 $temp3[] = $str;
						}
					}
				}
				$alltags = join(',',$temp3);
				if(mb_strlen($alltags, 'utf8') > maxTagsLength) {
					$linkres->tags = utf8_substr($alltags, 0, utf8_strrpos(utf8_substr($alltags, 0, maxTagsLength, 'utf-8'), ','));
				}else{
					$linkres->tags = $alltags;
				}
			}
//----------------------------------------------------------------------
...
{config_load file=rss_import_plikli_lang_conf}