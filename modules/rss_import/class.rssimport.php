...
// The plikli Team <pliklier at plikli dot com>. 	 
...
class RSSImport {
 ...
 //---------------------------------
	...
	function EIP_plikli_Field_Select(){

		$plikli_Fields = $this->get_plikli_fields();
		$TheTextToReturn = "	options: {";
		$Count = 0;
		if($plikli_Fields){
	...
	function read_feed_link(){
		...
		$config = $db->get_row($sql);
			...
			$this->FeedLinkplikli=$config->plikli_field;
	...
	function store_feed_link(){
		global $db;
		$sql = "UPDATE " . table_prefix . "feed_link set feed_id = '".$this->FeedId."', feed_field = '".$this->FeedLinkField."', plikli_field = '".$this->FeedLinkplikli."' where feed_link_id = ".$this->FeedLinkId.";";
		echo "<HR>" . $sql . "<HR>";
		$db->query($sql);
	}
	...
	function new_field_link(){
		global $db;
		$sql = "INSERT INTO " . table_prefix . "feed_link (feed_id, feed_field, plikli_field) VALUES(".$this->FeedLinkId.", '".$this->FeedLinkField."', '".$this->FeedLinkplikli."');";
		echo "<HR>" . $sql . "<HR>";
		$db->query($sql);
	}
