...
/*if ($_GET['unsub'] && $_GET['unsub'] == 1)
{
	...
	if($result)
	{
	    $decode=md5($result->user_email . $result->user_karma .  $username. kliqqi_hash());
	    if($uns_code==$decode)
	    {
		$linkres=new Link;
		$linkres->id = $link_id;
		if ($linkres->read())
		{
			...
			$main_smarty->assign('error', $main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_Unsubscribed_Message'));
		}		
	        else
			$main_smarty->assign('error', $main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_Invalid_Code'));
	    }
	    else
		$main_smarty->assign('error', $main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_Invalid_Code'));
	}
	else
	    $main_smarty->assign('error', $main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_No_User'));
...
if ($_REQUEST['uns'])
{
    ...
    echo "<span style=\"margin:15px 0 2px 0;\" class=\"btn btn-primary\" href=\"javascript://\" onclick=\"subscribe_2_comments({$_REQUEST['linkid']},{$_REQUEST['linkid']});\" ><i class=\"fa fa-envelope\"></i> ".$main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_Subscribe')."</span>";
...
else
{
    ...
    echo "<span style=\"margin:15px 0 2px 0;\" class=\"btn btn-primary\" href=\"javascript://\" onclick=\"subscribe_2_comments({$_REQUEST['linkid']},{$_REQUEST['linkid']},1);\" ><i class=\"fa fa-envelope\"></i> ".$main_smarty->get_config_vars('KLIQQI_Subscribe_2_Comments_Unsubscribe')."</span>";
